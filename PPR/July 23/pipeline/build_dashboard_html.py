...
function renderCurrent(){
  ...
  let h="<thead><tr><th class='grp metric-h cat-col' rowspan='2'>Category</th>"+
        "<th class='grp metric-h' rowspan='2'>Metric</th>"+
        `<th class='grp ctr'>${center}</th>`+
        `<th class='grp bench' colspan='${CT.length}'>All ATCs, Launch to Date</th></tr><tr>`+
        "<th class='lab-h'>Launch to Date</th>"+
        CT.map(c=>`<th class='lab-h bench'>${c}</th>`).join("")+
        "</tr></thead><tbody>";
  h+=bodyRows(m=>
    `<td class='val ltd'>${cval(m.metric,"Launch to Date")||"-"}</td>`+
    CT.map(c=>`<td class='val bench-col'>${qval(m.metric,c)||"-"}</td>`).join(""));
  document.getElementById("matrix").innerHTML=h+"</tbody>";
}
...
function render(){
  document.getElementById("colToggles").style.display = view==="proposed" ? "" : "none";
  if(view==="proposed")renderProposed();else renderCurrent();
  renderNotes();
}
render();
</script>
