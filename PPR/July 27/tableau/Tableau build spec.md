...
and Kolin's real per-center decks (`../Goal/Real PPR slides - Kolin (transcribed 2026-07-26).md`).

## CHANGES 2026-07-26 (read first if you built the earlier version)
1. **Per-metric event dating.** Every year/quarter/window column now counts each metric on
   ITS OWN event date, not the patient's enrollment date. Enrollments count by enrollment
   date, TTP metrics by pickup date, OOS by delivery date, infusions and timelines by
   infusion date. This is what Kolin's decks do ("Timing metrics based upon the TTP or
   Infusion Date"). The `Scorecard` extract already carries the corrected values, so the
   Proposed Template worksheet needs no rebuild, just the refreshed extract.
   Consequence: **Launch-to-Date no longer equals 2024 + 2025 + 2026** for a metric, because
   events with a null or out-of-range event date sit in Launch-to-Date but no year column.
   The old QA check that summed them is removed.
2. **New: date-window verification view** (section E). Reproduces Kolin's Year-over-Year
   slide and lets you match a slide by setting its two date windows. Built on `Orders`.
3. **Current Template** target is now the quartile-RANGE layout from Kolin's real slide
   (section B2): four range columns worst -> best with the center cell heat-colored. The
   percentile-value build still works as a simpler interim.
