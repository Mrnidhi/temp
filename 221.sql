...
WITH first_site AS (
    SELECT
        t.D_PATIENT_ID,
        c.CLASS_FINAL,
        FIRST_VALUE(t.D_PRIMARY_HCO_COMPILE_ID) OVER (
            PARTITION BY t.D_PATIENT_ID
            ORDER BY t.DATE_OF_SERVICE, t.D_PRIMARY_HCO_COMPILE_ID
        ) AS FIRST_HCO,
        FIRST_VALUE(t.IS_ATC_HCO) OVER (
            PARTITION BY t.D_PATIENT_ID
            ORDER BY t.DATE_OF_SERVICE, t.D_PRIMARY_HCO_COMPILE_ID
        ) AS STARTED_ATC
    FROM COMPILE_DEV.PUBLIC.ATC_TREATMENT_CLAIMS t
    INNER JOIN COMPILE_DEV.PUBLIC.ATC_CLASSIFIED_FINAL c
        ON t.D_PATIENT_ID = c.D_PATIENT_ID
    QUALIFY ROW_NUMBER() OVER (PARTITION BY t.D_PATIENT_ID
                               ORDER BY t.DATE_OF_SERVICE, t.D_PRIMARY_HCO_COMPILE_ID) = 1
),
starters AS (
    -- patients who started at a non-ATC site
    SELECT
        fs.FIRST_HCO,
        cl.HCO_PARENT_NAME AS STARTING_PARENT,
        fs.D_PATIENT_ID,
        CASE WHEN fs.CLASS_FINAL = 'ATC' THEN 1 ELSE 0 END AS MIGRATED
    FROM first_site fs
    LEFT JOIN COMPILE_DEV.PUBLIC.ATC_CLASSIFIED_FINAL cl
        ON fs.FIRST_HCO = cl.D_PRIMARY_HCO_COMPILE_ID
    WHERE fs.STARTED_ATC = 0
)
SELECT
    STARTING_PARENT,
